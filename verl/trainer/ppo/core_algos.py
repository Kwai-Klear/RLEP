...
def compute_policy_loss(
    ...
    negative_approx_kl = log_prob - old_log_prob
    ratio = torch.exp(negative_approx_kl)
    cispo_mask = torch.ones_like(advantages)
    cispo_mask[(advantages > 0) & (ratio > 1 + cliprange_high)] = 0.0
    # cispo_mask[(advantages < 0) & (ratio < 1 - cliprange_low)] = 0.0
    ...
    # mask out overly high updates
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    # pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask * cispo_mask, loss_agg_mode=loss_agg_mode)
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
