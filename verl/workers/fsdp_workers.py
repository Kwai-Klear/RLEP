...
class ActorRolloutRefWorker(Worker):
        ...
        with self.rollout_sharding_manager:
            ...
            if self.config.rollout.name == "sglang_async":
                ...
                if 'n' in prompts.meta_info:
                    args = {'n': prompts.meta_info['n']}
                if 'max_response_length' in prompts.meta_info:
                    args['max_tokens'] = prompts.meta_info['max_response_length']
                output = self.rollout.generate_sequences(prompts=prompts, **args)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_rollout_n_as_1(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_torch_device().current_device())
        ...
        with self.rollout_sharding_manager:
            ...
            if self.config.rollout.name == "sglang_async":
                ...
                output = self.rollout.generate_sequences(prompts=prompts, kwargs={'n': 1})
            log_gpu_memory_usage("After rollout generation", logger=logger)
